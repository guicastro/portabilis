					...
					if(count($RestrictedTables)>0) {
						...
						foreach ($RestrictedTables as $key => $RestrictedTablesObject) {
							...
							foreach ($RestrictedTablesObject as $Table => $Field) {
								...
								if((substr_count($AddJoin, $Table.".")>0)&&($Table<>$ModuleDefsTable)) {
						...
						/*### ADICIONA A RESTRIÇÃO, NA TABELA PRINCIPAL DO MÓDULO ###*/
						foreach ($RestrictedTables as $key => $RestrictedTablesObject) {

							if($RestrictedTablesObject->$ModuleDefsTable) {

								//ADICIONA AS CONDIÇÕES WHERE (TODOS OS REGISTROS)
								$SQLRecordsTotal .= " AND ".$this->ModuleDefs["Table"].".".$RestrictedTablesObject->$ModuleDefsTable." IN (".substr($SQLINParceiros,0,-2).") ";

								//ADICIONA AS CONDIÇÕES WHERE (REGISTROS FILTRADOS)
								$SQLRecordsFiltered .= " AND ".$this->ModuleDefs["Table"].".".$RestrictedTablesObject->$ModuleDefsTable." IN (".substr($SQLINParceiros,0,-2).") ";

								//ADICIONA AS CONDIÇÕES WHERE (TOTALIZADOR REGISTROS FILTRADOS)
								$SQLTotalRecordsFiltered .= " AND ".$this->ModuleDefs["Table"].".".$RestrictedTablesObject->$ModuleDefsTable." IN (".substr($SQLINParceiros,0,-2).") ";
							}
						}
						/*### ADICIONA A RESTRIÇÃO, NA TABELA PRINCIPAL DO MÓDULO ###*/
							...
							foreach ($RestrictedJoinTables as $key => $Table) {
								...
								foreach ($RestrictedTables as $key => $RestrictedTablesObject) {
									...
									if($RestrictedTablesObject->$Table) {
										...
										//ADICIONA AS CONDIÇÕES WHERE (TODOS OS REGISTROS)
										$SQLRecordsTotal .= " AND ".$Table.".".$RestrictedTablesObject->$Table." IN (".substr($SQLINParceiros,0,-2).") ";

										//ADICIONA AS CONDIÇÕES WHERE (REGISTROS FILTRADOS)
										$SQLRecordsFiltered .= " AND ".$Table.".".$RestrictedTablesObject->$Table." IN (".substr($SQLINParceiros,0,-2).") ";

										//ADICIONA AS CONDIÇÕES WHERE (TOTALIZADOR REGISTROS FILTRADOS)
										$SQLTotalRecordsFiltered .= " AND ".$Table.".".$RestrictedTablesObject->$Table." IN (".substr($SQLINParceiros,0,-2).") ";
									}
								}
							}
						}
