...
class Matricula extends \Database\Crud {
	...
	public function setSQLSelectFields () {
		...
		$this->SQLSelectFields = "SELECT TBL.*, alunos.*, cursos.*, periodos.* "
		...
		if($this->Action=="inserir") {
			...
			$SQLChecaMatricula = "SELECT
									...
									WHERE
										...
										AND cursos_curs_id = ".$this->Request["cursos_curs_id"]."
			...
			if($ResultSQLChecaMatricula[0]->matr_id > 0) {
				...
				$this->ErrorBeforeExecuteAction = "<p>O aluno escolhido já possui uma matrícula ATIVA no mesmo curso, período e ano letivo</p>";
			} else {
